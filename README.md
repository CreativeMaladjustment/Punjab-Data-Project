...
Add a second full set of secrets to the same `b2-upload` environment, suffixed `_2`, once a
bucket fills up (storage or a daily transaction cap) and you've created a second bucket,
possibly under a whole separate Backblaze account, to keep going:
...
in the script), that's treated as every account being genuinely stuck rather than a one-off
blip, and the whole run stops early (exit code 43, flagged in the job summary) instead of
spending the rest of the runtime budget failing the same way.

- Postgres records exactly which account/bucket holds each page's image and (separately) its
  optional page PDF, so a PDF's pages can legitimately end up split across both accounts with
  no ambiguity — there's never a need to redo a whole PDF just because pages landed on
  different accounts.
- **`extract_with_llm.py`** doesn't write to B2 at all, and already fetches each page from
  whichever account/bucket Postgres says holds it (`pages.b2_account`/`b2_bucket`), regardless
  of which account uploaded it. Extraction dedup (has this page already been processed by this
  model?) is tracked per page in `llm_extractions`, independent of B2 accounts entirely.
...
1. **Audit** (`scripts/audit_b2_pages.py`) — read-only (never writes to B2 or Postgres), lists
   ...
   image actually is, and any page whose image was uploaded to more than one B2 account (mostly
   a legacy condition from before `process_pcloud.py` tracked upload state in Postgres — see
   "Second B2 account" above). Exits non-zero if it finds anything — this step is allowed
