"""Extract catalogue entries from rendered page images using Hugging Face's
Inference Providers router, as another additional model alongside the
local-Ollama vision models (extract_with_llm.py) and Gemini
(extract_with_gemini.py). Reads the page images process_pcloud.py already
...
Why a separate script rather than a code path inside extract_with_llm.py or
extract_with_gemini.py: same reasoning as extract_with_gemini.py's own
docstring -- a hosted API has its own auth, request shape, and failure
modes that don't belong mixed into either of those scripts' control flow.

Namespaced under model_tag = slugified HF_MODEL (e.g. "Qwen-Qwen2.5-VL-7B-
Instruct") -- an ordinary peer in the same llm_extractions/model_tag
bake-off as glm-ocr, gemini-3.1-flash-lite, etc., not a reserved namespace.

What's different about Hugging Face vs. Gemini, and why this script's shape
differs from extract_with_gemini.py's:

  - Calls go through HF's OpenAI-compatible router
    (https://router.huggingface.co/v1/chat/completions), which fans out to
    whichever third-party Inference Provider (DeepInfra, Together,
    Fireworks, etc.) currently hosts HF_MODEL, rather than one fixed
    first-party endpoint the way Gemini's API is. Request/response shape
    follows OpenAI's chat-completions convention (messages with
    image_url content parts, choices[0].message.content), not Gemini's
    contents/parts shape.
  - The real constraint is a **dollar credit**, not a request-rate ceiling:
    a free HF account gets a small monthly Inference Providers credit
    (currently $0.10; see HF_EXTRACTION.md), refreshed monthly, not a
    per-minute/per-day request quota the way Gemini's free tier is. Once
    it's spent, the router returns 402 Payment Required -- unlike a 429
    (rate limit, worth retrying) or Gemini's 404 (wrong model name), a 402
    ...
    size it against.
  - Single worker, not a matrix, for the same reason as
    extract_with_gemini.py -- N parallel workers would all draw against
    the SAME shared monthly credit, so more workers just means the credit
    (and the run) runs out faster, not more pages processed for it.
  - A full-page OCR pass runs too (see hf_ocr_full_page(), mirroring
    extract_with_llm.py's ocr_full_page() and extract_with_gemini.py's
    gemini_ocr_full_page()) -- a second full API call per page, which
    roughly halves how many pages the monthly credit reaches. Accepted
    deliberately, same tradeoff already made for Gemini: page_ocr_text is
...
extract_with_gemini.py's own -- see those modules' docstrings for the full
reasoning; duplicated here rather than imported for the same reason
parse_ocr_text.py and extract_with_gemini.py duplicate them.
"""
import base64
import contextlib
import json
...
# is (there isn't one here; the real constraint is the dollar credit, see
# InsufficientCreditError below).
PACE_SECONDS = float(os.environ.get("HF_PACE_SECONDS", "2.0"))
...
# this can rescue an Ollama model's (or a Gemini model's) capped content
# failures exactly the same way.
...
if SOURCE_MODEL_TAG is not None and SOURCE_MODEL_TAG == MODEL_TAG:
    raise ValueError(
        f"SOURCE_MODEL ({SOURCE_MODEL!r}) slugifies to the same tag as HF_MODEL "
        f"({HF_MODEL!r}) -- a model can't rescue its own capped content failures "
        "under its own tag; set HF_MODEL to a different model"
    )
...
# the workflow gives this its own step-summary message (see
# extract-pages-hf.yml) so it isn't confused with a runtime-limited run.
CREDIT_EXHAUSTED_EXIT_CODE = 43
MAX_PAGES_PER_WORKER = int(os.environ.get("MAX_PAGES_PER_WORKER", "0"))
START_TIME = time.time()

HF_CONNECT_TIMEOUT_SECONDS = 10
HF_READ_TIMEOUT_SECONDS = 120  # a hosted API; generous but nowhere near
...
HF_SYSTEM_PROMPT = """You transcribe entries from a scanned page of a British \
...
# GEMINI_OCR_PROMPT exactly -- see there for the full reasoning.
HF_OCR_PROMPT = """Transcribe every word of text visible on this page \
...
def claim_next_page():
    ...
    script only ever runs against the one HF_MODEL its process is
    configured for."""
    params = {
        "model_tag": MODEL_TAG,
        "model": HF_MODEL,
...
class InsufficientCreditError(RuntimeError):
    """Raised on HTTP 402 from the router -- the monthly Inference
    Providers credit is spent. Deliberately NOT a subclass retried by
...
# Same reasoning as extract_with_gemini.py's GEMINI_RETRYABLE_STATUS_CODES:
# 429 and 5xx are transient by convention (worth retrying with backoff).
# 402 (insufficient credit) is deliberately NOT here -- see
# InsufficientCreditError above, waiting doesn't fix it.
HF_RETRYABLE_STATUS_CODES = {429, 500, 502, 503, 504}

HF_ROUTER_URL = "https://router.huggingface.co/v1/chat/completions"


def _hf_post(body, context):
    """Shared POST-with-pace-and-retry core for both hf_generate() and
    hf_ocr_full_page() below -- pacing and backoff apply per HTTP call, not
    per logical operation, and both make one call each, so they share this
    rather than each keeping its own copy of the retry loop. Returns the
    model's raw text response; raises (no .raw_text attached, i.e.
    transient by is_content_failure()'s convention) on anything short of a
    clean 200 with message content in it. Raises InsufficientCreditError
    (a distinct, non-retried condition) on 402."""
    headers = {
    ...
    for attempt in range(1, HF_MAX_RETRIES + 1):
        pace()
        resp = requests.post(
            HF_ROUTER_URL, json=body, headers=headers,
            timeout=(HF_CONNECT_TIMEOUT_SECONDS, HF_READ_TIMEOUT_SECONDS),
        )
        ...
        raise RuntimeError(f"HF router kept returning {resp.status_code} after {HF_MAX_RETRIES} retries")

    if not resp.ok:
        # Anything other than a clean 200 (auth, bad request, 5xx) is
        # transient/infra by the same convention the other extraction
        # scripts use -- retried indefinitely (next run), not capped.
        raise RuntimeError(f"HF router returned {resp.status_code}: {resp.text[:2000]}")

    payload = resp.json()
    try:
        return payload["choices"][0]["message"]["content"]
    except (KeyError, IndexError) as exc:
        raise RuntimeError(f"HF router response had no message content: {json.dumps(payload)[:2000]}") from exc
...
def hf_generate(image_bytes, context=""):
    """The structured-extraction call: schema-constrained prompt over
    OpenAI-style chat messages. Returns (entries, raw_text); raises with
    .raw_text attached on a parse/shape failure, same contract as
    extract_with_llm.py's extract_page() and extract_with_gemini.py's
    gemini_generate()."""
    body = {
        "model": HF_MODEL,
        "max_tokens": 4096,
    ...
    started = time.time()
    raw_text = _hf_post(body, context)
    text = raw_text.strip()
    text = re.sub(r"^```(json)?|```$", "", text, flags=re.M).strip()
    print(f"    HF response for {context} in {time.time() - started:.1f}s total ({len(text)} chars): {text[:300]!r}")
    try:
...
def hf_ocr_full_page(image_bytes, context=""):
    """Independent of hf_generate(): a verbatim transcription of everything
    on the page, not just the catalogue entries the schema-constrained
    prompt asks for. See extract_with_llm.py's ocr_full_page() /
    extract_with_gemini.py's gemini_ocr_full_page() for the full
    reasoning -- identical shape, no system message (the OCR prompt is the
    only instruction this call needs)."""
    body = {
        "model": HF_MODEL,
        "max_tokens": 4096,
        "messages": [
...
def db_save_extraction_success(page_id, entries, raw_text):
    with db_connection() as conn, conn.cursor() as cur:
        cur.execute(
            ...
            "RETURNING id",
            (page_id, HF_MODEL, MODEL_TAG, Json(entries), raw_text),
        )
...
def db_save_extraction_failure(page_id, error_message, raw_text, content_failure):
    with db_connection() as conn, conn.cursor() as cur:
        cur.execute(
            ...
            "created_at = now()",
            (page_id, HF_MODEL, MODEL_TAG, error_message, raw_text, content_failure),
        )
        conn.commit()
...
def process_page(clients, claim):
    """Extract one already-claimed page. See extract_with_gemini.py's
    process_page() -- identical shape, including the full-page OCR call.
    InsufficientCreditError is deliberately NOT caught here: it propagates
    up to main(), which stops the whole run rather than marking this one
    page a content failure (the credit is what ran out, not this page's
    extraction) -- the page's claim just goes stale and is retried once
    the credit renews next month."""
    ...
    try:
        ...
        # success/failure/retry accounting, exactly like the other two
        # extraction scripts' process_page() treats their own OCR call. A
        # credit-exhaustion 402 here still propagates (it isn't this
        # page's fault either way, and the structured-extraction call
        # below would just fail the same way immediately after).
        try:
            page_text = hf_ocr_full_page(image_bytes, context=context)
            db_save_page_ocr_text(page_id, HF_MODEL, MODEL_TAG, page_text)
        except InsufficientCreditError:
            raise
        except Exception as exc:
            print(f"WARNING: full-page OCR failed for {context}: {exc}")
            db_save_page_ocr_failure(page_id, HF_MODEL, MODEL_TAG, str(exc))
        entries, raw_text = hf_generate(image_bytes, context=context)
        for entry in entries:
...
def main():
    ...
    print(f"model: {HF_MODEL} (tag: {MODEL_TAG}); pace: {PACE_SECONDS}s between requests")
    if SOURCE_MODEL_TAG:
    ...
    while True:
        ...
        try:
            ...
            print(f"{exc}")
            print("stopping run: this month's HF Inference Providers credit is spent; "
                  "this page's claim will go stale and be retried once the credit renews")
